...
*   **Secondary Accent:** `Electric Violet (#8B5CF6)` - Represents communication, queries, and active signals.
...
2.  **Agent Satellites:**
    ...
    *   **States:**
        ...
        *   `THINKING`: Pulsing Violet outer ring.
...
*   **Insight Pulse (Animation):**
    ...
    *   **Query:** A Violet "wave" travels from the Hive Mind to the Agent Node.
    *   **Signal Flash:** When `priority = 5`, a Violet ripple expands from the source agent, momentarily illuminating all connected nodes.
...
function triggerPulse(sourceId, targetId, type) {
    const color = type === 'CONTRIBUTION' ? '#D4AF37' : '#8B5CF6';
