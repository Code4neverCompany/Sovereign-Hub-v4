...
- **Electric Cyan (#00FFFF):** The neural pulse. Used for 'Heat Markers', code additions in diffs, and active data streams.
...
### Visual Architecture:
- **Track:** A thin, semi-transparent cyan line (`rgba(0, 255, 255, 0.1)`) spanning the width of the Trace panel.
- **Heat Markers:** Small, glowing cyan pips (shadow: `0 0 10px #00FFFF`) clustered where tool calls or state changes occur.
...
- **Code Highlighting:**
  - **Additions:** Cyan text with `rgba(0, 255, 255, 0.1)` background.
...
<section id="trace-tab" class="tab-panel">
    <div class="chrono-interface h-full flex flex-col gap-6">
        ...
        <div class="scrubber-container glass-card p-6 h-32 relative flex flex-col justify-center">
            <div class="timeline-track h-1 bg-cyan/10 rounded-full relative">
        ...
        <div class="grid grid-cols-2 gap-6 flex-1 overflow-hidden">
            ...
            <div class="diff-pane glass-card p-6 flex flex-col border-cyan/10">
                <h4 class="text-[10px] text-cyan/40 uppercase font-mono mb-4">Reconstituted_Logic</h4>
