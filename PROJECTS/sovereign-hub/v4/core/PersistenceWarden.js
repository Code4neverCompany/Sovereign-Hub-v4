...
export class PersistenceWarden {
    ...
    updateUI() {
        ...
        document.querySelectorAll('.warden-pulse-status').forEach(el => {
            ...
            el.className = `warden-pulse-status text-[8px] font-mono uppercase tracking-widest mt-1 ${this.status === 'SYNCING' ? 'text-cyan animate-pulse' : (this.status === 'RELEASE_READY' ? 'text-gold' : 'text-cyan/60')}`;
        });
        ...
        document.querySelectorAll('.persistence-ring').forEach(el => {
            ...
            el.style.stroke = this.status === 'SYNCING' ? '#00FFFF' : '#D4AF37';
        });
    ...
    getStatusStyle(status) {
        ...
        if (status === 'SYNCING') return 'border-cyan/20 text-cyan animate-pulse';
