...
async function check() {
    console.log("🔱 Fetching 'todos' metadata...")
    const { data, error } = await supabase.from('todos').select('*').limit(1)
    if (data && data.length > 0) {
        console.log("✅ Columns found in existing data:", Object.keys(data[0]))
    } else {
        console.log("❌ Table is empty or error:", error ? error.message : "No data")
    }
}
