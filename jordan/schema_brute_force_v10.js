...
async function testInsert() {
  ...
  const priorities = ['High', 'Medium', 'Low', 'high', 'medium', 'low', 'Highest', 'Lowest', 'Routine', 'Critical', 'Major', 'Minor', 'Blocker', 'Task', 'Bug', '1', '2', '3', 'HIGH', 'MEDIUM', 'LOW', 'URGENT', 'NORMAL']
  ...
  for (const category of categories) {
    for (const priority of priorities) {
      ...
      const { data, error } = await supabase
        .from('todos')
        .insert([{ title: 'Test', category, priority, status: 'Active' }])
        .select()
      if (!error) {
        console.log('SUCCESS!')
